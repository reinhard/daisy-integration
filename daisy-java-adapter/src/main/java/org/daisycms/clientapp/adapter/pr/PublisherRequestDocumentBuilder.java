 ...
 * under the License. 
 */
package org.daisycms.clientapp.adapter.pr;
...
public class PublisherRequestDocumentBuilder {
    ...
    public static PublisherRequestDocument createPublisherRequestDocument(final DaisyDocumentProxy proxy, final PRDocumentConfiguration config) {
        final long id = proxy.getDocId();
        ...
        Validate.isTrue(id > 0, "You have to pass the document id.");
        ...
        if(!config.isNavigationDocument) {
            ...
            if(config.annotatingNavDocId > 0) {
