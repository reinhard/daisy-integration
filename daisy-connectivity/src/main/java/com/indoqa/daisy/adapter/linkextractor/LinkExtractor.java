...
public class LinkExtractor {
     ...
     */
    public static DaisyLink[] extract(final DaisyDocument doc) throws XmlException, IOException {
     ...
     * @param parent
     */
    private static DaisyLink[] extractDaisyHtmlLinks(final DaisyDocument doc) throws XmlException, IOException {
        List<DaisyLink> daisyLinks = new ArrayList<DaisyLink>();

        // find all links in 'p:linkInfo'
        XmlObject x = XmlObject.Factory.parse(doc.getInputStream());
        XmlCursor c = x.newCursor();

        String queryExpression = "declare namespace p='http://outerx.org/daisy/1.0#publisher';" + "$this//p:linkInfo";
        ...
        c = x.newCursor();
        queryExpression = "declare namespace d='http://outerx.org/daisy/1.0';" + "$this//d:searchResult/d:rows/d:row";
        c.selectPath(queryExpression);
        ...
        return daisyLinks.toArray(new DaisyLink[daisyLinks.size()]);
    }
     ...
     * @param parent
     */
    private static DaisyLink[] extractNavigationTreeLinks(final DaisyDocument doc) throws XmlException, IOException {
        List<DaisyLink> daisyLinks = new ArrayList<DaisyLink>();
        XmlObject x = XmlObject.Factory.parse(doc.getInputStream());
        XmlCursor c = x.newCursor();

        String queryExpression = "declare namespace n='http://outerx.org/daisy/1.0#navigation';" + "$this//n:doc";
        c.selectPath(queryExpression);
        while (c.hasNextSelection()) {
            c.toNextSelection();
